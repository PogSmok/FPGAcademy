...
_start:	
	...
	li a1, 0	 # a1 stores current high digit
    	...
    	li s0, 100000000  # 1 second timeout
    	sw s0, 0(t5)      # mtimecmp low
    	sw x0, 4(t5)      # mtimecmp high

   	sw x0, 0(s1)      # reset mtime low
    	sw x0, 4(s1)      # reset mtime high
...
wait_timer:
	...
	sw a2, (t0)
cont:
	lw t6, 0(s1)     # read mtime low
    	blt t6, s0, wait_timer

    	sw zero, 0(s1)   # reset mtime low
    	sw zero, 4(s1)   # reset mtime high	
