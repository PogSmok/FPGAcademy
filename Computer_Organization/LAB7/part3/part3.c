...
void KEY_ISR(void) {
  if(KEY_REGISTER->edgecapture & KEY0_MASK) {
    run ^= 1;
  } 
  
  if(KEY_REGISTER->edgecapture & KEY1_MASK) {
    MACHINE_TIMER->mtcmp_lo >>= 1;
    if(MACHINE_TIMER->mtcmp_hi & 1) MACHINE_TIMER->mtcmp_lo |= 0x80000000;
    MACHINE_TIMER->mtcmp_hi >>= 1;
  }
  
  if(KEY_REGISTER->edgecapture & KEY2_MASK) {
    MACHINE_TIMER->mtcmp_hi <<= 1;
    if(MACHINE_TIMER->mtcmp_lo & 0x80000000) MACHINE_TIMER->mtcmp_hi |= 1;
    MACHINE_TIMER->mtcmp_lo <<= 1;
  }
